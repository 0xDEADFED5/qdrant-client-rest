...
class AsyncQdrantRemote(AsyncQdrantBase):
        ...
        **kwargs: Any,
        ...
        if host is not None and (host.startswith("http://") or host.startswith("https://")):
            ...
            if self._prefix and parsed_url.path:
                raise ValueError(f"Prefix can be set either in `url` or in `prefix`. url is {url}, prefix is {parsed_url.path}")
        ...
        self.openapi_client: AsyncApis[AsyncApiClient] = AsyncApis(host=self.rest_uri, **self._rest_args)
    ...
    async def search_batch(
        ...
        **kwargs: Any,
        ...
        # else:
        requests = [
            # GrpcToRest.convert_search_points(r) if isinstance(r, grpc.SearchPoints) else
            r
            for r in requests
        ]
    ...
    async def query_batch_points(
        ...
        **kwargs: Any,
        ...
        # else:
        requests = [
            # GrpcToRest.convert_query_points(r) if isinstance(r, grpc.QueryPoints)
            # else
            r
            for r in requests
        ]
    ...
    async def search_groups(
        ...
        **kwargs: Any,
        ...
        #     with_lookup = GrpcToRest.convert_with_lookup(with_lookup)
        if isinstance(query_vector, tuple):
            query_vector = construct(models.NamedVector, name=query_vector[0], vector=query_vector[1])
        if isinstance(query_vector, np.ndarray):
            query_vector = query_vector.tolist()
        # if isinstance(query_filter, grpc.Filter):
        #     query_filter = GrpcToRest.convert_filter(model=query_filter)
    ...
    async def discover_batch(
        ...
        **kwargs: Any,
        ...
        # else:
        requests = [
            # GrpcToRest.convert_discover_points(r) if isinstance(r, grpc.DiscoverPoints) else
            r
            for r in requests
        ]
        http_res: list[list[models.ScoredPoint]] = (
        ...
        **kwargs: Any,
        ...
        count_result = (
            await self.openapi_client.points_api.count_points(
                collection_name=collection_name,
                count_request=models.CountRequest(filter=count_filter, exact=exact, shard_key=shard_key_selector),
                timeout=timeout,
            )
        ).result
        ...
        **kwargs: Any,
        ...
        return (
            await self.openapi_client.points_api.update_vectors(
                collection_name=collection_name,
                wait=wait,
                update_vectors=models.UpdateVectors(points=points, shard_key=shard_key_selector),
                ordering=ordering,
            )
        ).result
    ...
    async def retrieve(
        ...
        **kwargs: Any,
        ...
        #     with_payload = GrpcToRest.convert_with_payload_selector(with_payload)
        ids = [
            # GrpcToRest.convert_point_id(idx) if isinstance(idx, grpc.PointId) else
            idx
            for idx in ids
        ]
        http_result = (
    ...
    def _try_argument_to_rest_selector(
        ...
        if isinstance(points, list):
            ...
            points_selector = construct(models.PointIdsList, points=_points, shard_key=shard_key_selector)
        # elif isinstance(points, grpc.PointsSelector):
    ...
    def _try_argument_to_rest_points_and_filter(
        ...
        if isinstance(points, models.PointIdsList):
        ...
        **kwargs: Any,
        ...
        result: Optional[types.UpdateResult] = (
            await self.openapi_client.points_api.delete_payload(
                ...
                wait=wait,
                ordering=ordering,
                delete_payload=models.DeletePayload(keys=keys, points=_points, filter=_filter, shard_key=shard_key_selector),
            )
        ).result
    ...
    async def get_aliases(self, **kwargs: Any) -> types.CollectionsAliasesResponse:
        ...
        #         ]
        #     )
        result: Optional[types.CollectionsAliasesResponse] = (await self.http.aliases_api.get_collections_aliases()).result
    ...
    async def get_collections(self, **kwargs: Any) -> types.CollectionsResponse:
        ...
        #         ]
        #     )
        result: Optional[types.CollectionsResponse] = (await self.http.collections_api.get_collections()).result
    ...
    async def delete_collection(self, collection_name: str, timeout: Optional[int] = None, **kwargs: Any) -> bool:
        ...
        #         )
        #     ).result
        result: Optional[bool] = (await self.http.collections_api.delete_collection(collection_name, timeout=timeout)).result
    ...
    def upload_collection(
        ...
        **kwargs: Any,
        ...
        batches_iterator = self._updater_class.iterate_batches(vectors=vectors, payload=payload, ids=ids, batch_size=batch_size)
        self._upload_collection(
            batches_iterator=batches_iterator,
            collection_name=collection_name,
            max_retries=max_retries,
    ...
    async def list_snapshots(self, collection_name: str, **kwargs: Any) -> list[types.SnapshotDescription]:
        ...
        snapshots = (await self.openapi_client.snapshots_api.list_snapshots(collection_name=collection_name)).result
    ...
    async def create_snapshot(
        ...
        return (await self.openapi_client.snapshots_api.create_snapshot(collection_name=collection_name, wait=wait)).result
    ...
    async def create_full_snapshot(self, wait: bool = True, **kwargs: Any) -> types.SnapshotDescription:
        # if self._prefer_grpc:
        #     snapshot_description = (
    ...
    async def delete_full_snapshot(self, snapshot_name: str, wait: bool = True, **kwargs: Any) -> Optional[bool]:
        ...
        #     )
        #     return True
        return (await self.openapi_client.snapshots_api.delete_full_snapshot(snapshot_name=snapshot_name, wait=wait)).result
        ...
        **kwargs: Any,
        ...
        return (
            await self.openapi_client.snapshots_api.recover_from_snapshot(
                collection_name=collection_name,
                wait=wait,
                snapshot_recover=models.SnapshotRecover(location=location, priority=priority, checksum=checksum, api_key=api_key),
            )
        ).result

    async def list_shard_snapshots(self, collection_name: str, shard_id: int, **kwargs: Any) -> list[types.SnapshotDescription]:
        snapshots = (
            await self.openapi_client.snapshots_api.list_shard_snapshots(collection_name=collection_name, shard_id=shard_id)
        ).result
        assert snapshots is not None, "List snapshots API returned None result"
        return snapshots
    ...
    async def lock_storage(self, reason: str, **kwargs: Any) -> types.LocksOption:
        result: Optional[types.LocksOption] = (
            await self.openapi_client.service_api.post_locks(models.LocksOption(error_message=reason, write=True))
        ).result
    ...
    async def get_locks(self, **kwargs: Any) -> types.LocksOption:
        result: Optional[types.LocksOption] = (await self.openapi_client.service_api.get_locks()).result
